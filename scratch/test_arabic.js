...
// الحروف التي لا تتصل بما بعدها (على اليسار)
...
function shapeArabic(text) {
  ...
  // 1. معالجة لام الألف أولاً كحالة خاصة (Ligatures)
  let chars = [];
  ...
  // 2. تطبيق قواعد الاتصال لتحديد الشكل المناسب
  let shapedText = '';
  for (let i = 0; i < chars.length; i++) {
    ...
    // الاتصال باليمين (الحرف السابق يتصل باليسار والحالي يتصل باليمين)
    const connectBefore = prev && prev.isArabic && !NON_LEFT_CONNECTING.has(prev.char) && current.char !== '\u0621';

    // الاتصال باليسار (الحالي يتصل باليسار والتالي يتصل باليمين)
...
function reverseArabicLine(text) {
  ...
  // استعادة ترتيب الأرقام والنصوص الإنجليزية
  // نبحث عن كتل الأرقام والنصوص الإنجليزية وعلامات الترقيم الخاصة بها
...
// اختبار بعض الحالات الشائعة
const testCases = [
  'بيانات العميل',
  'الاسم: حضرمي ستايل',
  'الجوال: 777616165',
  'العنوان: الشرج المكلا الديس',
  'رقم الطلب: #TM-0064',
  'المنتج / الخدمة',
  '30 ر.س',
  'إجمالي المنتجات:',
...
for (const tc of testCases) {
  ...
  console.log(`Fixed:    "${reverseArabicLine(tc)}"`);
  console.log('--------------------');
}
