...
const fontsToTest = [
  ...
  { name: 'NotoSansArabic', file: 'NotoSansArabic.ttf' },
  { name: 'NotoNaskhArabic', file: 'NotoNaskhArabic.ttf' },
  { name: 'Alexandria', file: 'Alexandria-Regular.ttf' },
  { name: 'ReadexPro', file: 'Readex-Pro.ttf' }
];

let y = 20;
doc.setFontSize(16);
doc.text('Testing Arabic Fonts with jsPDF & shaping', 10, y);
y += 15;
...
console.log('Shaped & Reversed:', reversedText);
...
for (const font of fontsToTest) {
  ...
  try {
    ...
    doc.text(`Font: ${font.name}`, 15, y);
    y += 7;
    // Draw Arabic text
    doc.text(reversedText, 195, y, { align: 'right' });
    y += 15;
    console.log(`✅ Added ${font.name} to PDF`);
  } catch (err) {
    console.error(`❌ Failed to add ${font.name}:`, err.message);
  }
}

const buffer = doc.output('arraybuffer');
fs.writeFileSync(path.join(__dirname, '..', 'test_fonts_comparison.pdf'), Buffer.from(buffer));
console.log('Done! Generated test_fonts_comparison.pdf');
